...
def match_yolo_to_gt(
    yolo_boxes: Dict[int, List[float]],
...
def load_ground_truth(
    expression_path: str, labels_dir: str
) -> Tuple[str, Dict[str, list], Dict[int, list]]:
    """
...
def draw_frame_visualization(
    ...
    for yid in yolo_ids:
        ...
        cv2.putText(
            frame, label_text, (x1, y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.40, color, 1
        )
    ...
    cv2.putText(
        frame,
        f"Frame {frame_idx}/{total_frames} | GT tracks: {num_gt}",
        (10, 50),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (200, 200, 200),
...
def evaluate_frame_metrics(
    ...
    precision = (
        tp / (tp + fp) if tp + fp > 0 else (1.0 if num_gt_this_frame == 0 else 0.0)
    )
...
def run_e2e_evaluation(
    ...
    for frame_idx, frame_name in enumerate(frame_files):
        ...
        scores, velocities = linker.process_frame(
            frame, active_tracks, language_embedding, detections=xyxy
        )
        ...
        if gt_track_ids_this_frame and frame_idx in gt_labels:
            ...
            matched_yolo_ids = match_yolo_to_gt(
                yolo_boxes_dict, gt_bboxes, iou_threshold=0.3
            )
        else:
            matched_yolo_ids = set()
            gt_bboxes = []
        ...
        precision, recall, tp, fp, fn, gt_img_scores, non_gt_img_scores = (
        ...
        if (
            gt_track_ids_this_frame
        ):  # Only count frames that have GT for summary metrics
    ...
    if gt_scores:
        print(
            f"GT-matched tracks:     avg score = {np.mean(gt_scores):.4f} (n={len(gt_scores)})"
        )
    else:
    ...
    if non_gt_scores:
        print(
            f"Non-GT tracks:         avg score = {np.mean(non_gt_scores):.4f} (n={len(non_gt_scores)})"
        )
    else:
    ...
    if gt_scores and non_gt_scores:
        ...
        print(
            f"Score separation:      {separation:+.4f} {'✅' if separation > 0 else '❌'}"
        )
